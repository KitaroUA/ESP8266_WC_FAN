...
*/
void ICACHE_FLASH_ATTR ws2812_sendByte(uint8_t b)
{
   static uint8_t bit;
   static uint32_t out;
   for ( bit = 0; bit < 8; bit++)
...
void  ICACHE_FLASH_ATTR ws2812_sendPixel(uint8_t r, uint8_t g, uint8_t b)
{
...
void  ICACHE_FLASH_ATTR ws2812_showColor(uint16_t count, uint8_t r , uint8_t g , uint8_t b)
{
   uint16_t pixel;
...
void  ICACHE_FLASH_ATTR ws2812_colorTransition(  uint8_t from_r, uint8_t from_g, uint8_t from_b,
...
void  ICACHE_FLASH_ATTR ws2812_show()
{
   os_delay_us(9);
}
...
static void  ICACHE_FLASH_ATTR ws2812_timer_cb(void* arg)
{
...
bool  ICACHE_FLASH_ATTR ws2812_spi_init()
{
...
bool  ICACHE_FLASH_ATTR ws2812_init()
{
   if (ws2812_spi_init())
   {
      INFO("ws2812_spi_init ok\n");
...
// Display a single color on the whole string
void ICACHE_FLASH_ATTR ws2812_sendPixels()
{
...
void  ICACHE_FLASH_ATTR ws2812_showit_fade(void)
{
...
void  ICACHE_FLASH_ATTR ws2812_doit(void)
{
   switch (ws->cur_anim)
   {
...
static void  ICACHE_FLASH_ATTR ws2812_fade_cb(void)
{
...
void  ICACHE_FLASH_ATTR ws2812_anim_init(uint8_t anim_type)
{
   os_timer_disarm(&ws2812_timer);
...
void  ICACHE_FLASH_ATTR ws2812_set_brightness(uint8_t b)
{
   ws->brightness = b;
}

/* Stop currently running animation, if any. */
void  ICACHE_FLASH_ATTR ws2812_anim_stop(void)
{
   ws2812_clear();
   os_timer_disarm(&ws2812_timer);
